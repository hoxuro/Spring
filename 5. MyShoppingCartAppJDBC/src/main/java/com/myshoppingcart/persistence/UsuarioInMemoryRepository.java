...
public class UsuarioInMemoryRepository implements IUsuarioRepository {
    ...
    public boolean existeUsuario(String email, String password) {
        ...
        for(Usuario usuario: usuarios) {
            if(usuario.getEmail().equals(email) && usuario.getPassword().equals(password)) {
                existe = true;
                break; // I found the user, so I no longer want to search it
            }
        }
    ...
    @Override
    public Usuario updateUsuario(Usuario actUsuario) throws UsuarioNotFoundException {
        for (Usuario usuario : usuarios) {
            if (usuario.getUid() == actUsuario.getUid()) {
                usuarios.remove(usuario);
                usuarios.add(actUsuario);
                return actUsuario;
            }
        }
